...
fs.createReadStream(csvFilePath)
  ...
  .on('data', (row) => {
    ...
    console.log(
      `📝 Processing faculty ${facultyCount + 1}: ${row['First Name']} ${row['Last Name']} (ID: ${id})`,
    )
  ...
  .on('end', async () => {
    ...
    try {
      ...
      for (const [id, data] of Object.entries(faculty)) {
        ...
        uploadedCount++
        console.log(
          `✅ Uploaded faculty ${uploadedCount}/${Object.keys(faculty).length}: ${data.personalInfo.firstName} ${data.personalInfo.lastName} (${id})`,
        )
      }
