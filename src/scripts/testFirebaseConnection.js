...
console.log(`   Exists: ${fs.existsSync(serviceAccountPath) ? '✅ Yes (fallback available)' : '⚠️  No (will use default credentials)'}`);
...
fs.createReadStream(csvFilePath)
  ...
  .on('end', () => {
    ...
    // Summary
    const hasFirebaseConfig = process.env.VITE_FIREBASE_PROJECT_ID && process.env.VITE_FIREBASE_API_KEY;
    ...
    console.log(`   • Service Account Key: ${hasServiceAccount ? '✅ Available' : '⚠️  Not needed (using env vars)'}`);
