...
fs.createReadStream(csvFilePath)
  ...
  .on('data', (row) => {
    ...
    console.log(
      `📝 Processing parent ${parentCount + 1}: ${row['First Name']} ${row['Last Name']} (ID: ${id})`,
    )
  ...
  .on('end', async () => {
    ...
    try {
      ...
      for (const [id, data] of Object.entries(parents)) {
        ...
        uploadedCount++
        console.log(
          `✅ Uploaded parent ${uploadedCount}/${Object.keys(parents).length}: ${data.personalInfo.firstName} ${data.personalInfo.lastName} (${id})`,
        )
      }
