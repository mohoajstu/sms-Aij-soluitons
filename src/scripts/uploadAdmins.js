...
fs.createReadStream(csvFilePath)
  ...
  .on('data', (row) => {
    ...
    console.log(`📝 Processing admin ${adminCount + 1}: ${row['First Name']} ${row['Last Name']} (ID: ${id})`);
  ...
  .on('end', async () => {
    ...
    try {
      ...
      for (const [id, data] of Object.entries(admins)) {
        ...
        uploadedCount++;
        console.log(`✅ Uploaded admin ${uploadedCount}/${Object.keys(admins).length}: ${data.personalInfo.firstName} ${data.personalInfo.lastName} (${id})`);
      }
