...
import { cilPlus, cilPencil, cilTrash, cilSearch, cilFilter, cilCalendar, cilListRich, cilSpreadsheet } from '@coreui/icons-react';
...
const CoursesView = () => {
  ...
  const renderCoursesTable = () => {
    ...
    if (filteredCourses.length === 0) {
      return (
        <CAlert color="info">
          No courses found. {searchTerm || filterGrade || filterSubject ? 'Try adjusting your filters.' : ''}
        </CAlert>
      );
    }
          ...
          {filteredCourses.map(course => (
            <CTableRow key={course.id}>
              <CTableDataCell>
                ...
                {course.description && (
                  <div className="course-description">{course.description.substring(0, 100)}...</div>
                )}
              </CTableDataCell>
              ...
              <CTableDataCell>
                <div>
                  {course.schedule.classDays.map(day => day.substring(0, 3)).join(', ')}
                </div>
              ...
              <CTableDataCell>
                <div className="enrollment">
                  <span className={getEnrollmentColorClass(course.enrolledStudents, course.capacity)}>
