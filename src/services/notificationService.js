...
const FIREBASE_FUNCTIONS_URL = 'https://northamerica-northeast1-tarbiyah-sms.cloudfunctions.net/sendSmsHttp';
...
const NotificationService = {
   ...
   * @param {string} [data.customMessage] Optional custom message to send
   * @returns {Promise<Object>} Result of the API call
   */
  sendAbsenceNotification: async ({ phoneNumber, studentName, className, date, customMessage }) => {
    try {
      // Format the phone number if needed
      const formattedPhoneNumber = formatPhoneNumber(phoneNumber);
      
      // Use the custom message if provided, otherwise generate a default one
      const message = customMessage || `Ottawa Islamic School Attendance Alert: ${studentName} was marked absent from ${className || 'class'} on ${new Date(date).toLocaleDateString('en-US', {
        weekday: 'long',
        year: 'numeric',
        month: 'long',
        day: 'numeric'
      ...
      console.log(`Endpoint: ${FIREBASE_FUNCTIONS_URL}`);
